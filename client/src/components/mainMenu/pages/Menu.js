...
const Menu = ({ user, setUser, setData, logout }) => {
   let navigate = useNavigate()
   const { showModalConf, setShowModalConf } = useContext(ModalConfirmationContext)
   ...
   return (
      <>
         <div className="menu">
            ...
            {/* Quick Match With Id*/}
            <Button text="MATCH WITH ID" path="qm-type" disabled={loading} />
            {/* Ranked */}
            <Button
               text="RANKED MATCH"
         ...
         {/* Confirmation Modal */}
         {showModalConf && (
            <MainMenuConfirmation text={text} btn1={btn1} btn2={btn2} cancel={cancel} />
         )}
      </>
   )
}
