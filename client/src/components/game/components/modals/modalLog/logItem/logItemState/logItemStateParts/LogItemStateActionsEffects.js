...
import { getCards, getCorporationByName, getShortName } from '../../../../../../../../utils/misc'
...
const LogItemStateActionsEffects = ({ state, type }) => {
   ...
   const getActions = () => {
      let actions = []
      if (isUnmi) actions.push([0, getShortName(CORP_NAMES.UNMI), getActionIcon(ACTION_ICONS.ACTION_UNMI)])
      ...
      if (cards.length > 0) {
         cards.forEach((card) => {
            actions.push([card.id, getShortName(card.name), getActionIcon(card.iconNames.action)])
         })
      }
      return actions
   }
   const getEffects = () => {
      let effects = []
      if (!isUnmi) effects.push([0, getShortName(state.statePlayer.corporation.name), getEffectIcon(state.statePlayer.corporation.effects[0])])
      const cards = state.statePlayer.cardsPlayed.filter((card) =>
      ...
      if (cards.length > 0) {
         cards.forEach((card) => {
            effects.push([card.id, getShortName(card.name), getEffectIcon(card.effect)])
         })
      }
